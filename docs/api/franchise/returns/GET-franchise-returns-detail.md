...
- curl 명령 예시
  ...
  ```
  curl -i -X GET 'http://localhost:8080/api/v1/franchise/returns/RTN-20240101-001' \
...
- 요청자가 속한 가맹점의 반품만 조회할 수 있다 (타 가맹점 반품 조회 시 403)
...
- [ ] 타 가맹점의 반품을 조회하면 403 Forbidden 상태코드를 반환한다
- [ ] 존재하지 않는 return-code면 404 Not Found를 반환한다
- [ ] 반환된 반품의 returnCode가 요청한 return-code와 일치한다
