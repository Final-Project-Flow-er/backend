...
- curl 명령 예시
  ...
  ```
  curl -i -X PATCH 'http://localhost:8080/api/v1/hq/returns/RTN-20240101-001' \
...
- 본문
  ...
  ```json
  {
    ...
    "data": {
      ...
      "returnCode": "string",
      "status": "ACCEPTED | INSPECTING | COMPLETED | REJECTED",
      "productCode": "string",
      "type": "DEFECT | SURPLUS",
...
- 변경 가능한 상태: PENDING → ACCEPTED 또는 REJECTED, ACCEPTED → INSPECTING
- 이미 완료(COMPLETED)되거나 취소(CANCELED)된 반품의 상태는 변경할 수 없다
...
- [ ] COMPLETED 또는 CANCELED 상태의 반품 상태를 변경하면 예외를 반환한다
