...
public class UserManagementController {
    ...
    public ResponseEntity<ApiResponse<CreateUserResponse>> createUser(
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success(userManagementFacade.registerUser(request, profileImage, principal.getId())));
    }

    @Operation(summary = "회원 목록 조회", description = "전체 회원 목록 조회")
    @GetMapping
    public ResponseEntity<ApiResponse<Page<UserSummaryResponse>>> getUserList(
            UserSearchRequest request,
            @PageableDefault(size = 20) Pageable pageable
    ) {
        return ResponseEntity.ok(ApiResponse.success(userManagementFacade.getUserList(request, pageable)));
    }
    ...
    public ResponseEntity<ApiResponse<UserDetailResponse>> updateUser(
        ...
        UserDetailResponse response = userManagementFacade.updateUser(id, request, profileImage, principal.getId());
