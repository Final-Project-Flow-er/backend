...
public class InternalTransportController {
    ...
    public ApiResponse<AvailableVehicleResponse> getAvailableVehicles() {
        // TODO: 서비스 로직 연결
    ...
    @PostMapping("/assign")
    public ApiResponse<?> assignVehicle(@RequestBody VehicleAssignmentRequest req) {
        // TODO: 배정 로직 작성
        return ApiResponse.success("차량 배정이 완료되었습니다!");
    }

    @Operation(summary = "배차 해제", description = "배정된 차량 해제")
    @DeleteMapping("/assign/{transportId}")
    public ApiResponse<?> cancelAssignment(@PathVariable Long transportId) {
        // TODO: 해제 로직 작성
        return ApiResponse.success("배차가 해제되었습니다.");
    }

    @Operation(summary = "입고 승인 상태 변경", description = "입고 승인 시 상태 변경")
    @PatchMapping("/{transportId}/arrival-approve")
    public ApiResponse<?> approveArrival(@PathVariable Long transportId, @RequestBody ArrivalApprovalRequest req) {
        // TODO: 상태 변경 로직 (예: ARRIVED)
        return ApiResponse.success("입고 승인 처리가 되었습니다.");
    }

    @Operation(summary = "상태 강제 수정", description = "운송 및 차량 상태 강제 수정")
    @PutMapping("/{transportId}/force-update")
    public ApiResponse<?> forceUpdateStatus(@PathVariable Long transportId, @RequestBody TransportForceUpdateRequest req) {
        // TODO: 강제 수정 로직
