...
public class HQOrderFacade {
        ...
        public List<HQOrderResponse> getAllOrders() {
                ...
                Map<Long, String> phoneNumberByUserId = orders.values().stream()
                                .collect(Collectors.toMap(
                                                HQOrderCommand::userId,
                                                command -> userManagementService
                                                                .getPhoneNumberByUserId(command.userId()),
                                                (a, b) -> a));
                ...
                Map<Long, List<com.chaing.domain.orders.dto.command.HQOrderItemCommand>> orderItemsByOrderId = hqOrderService
                                .getOrderItemIdsByOrderId(orderIds);

                // Map<orderItemId, HQOrderItemCommand>
                Map<Long, com.chaing.domain.orders.dto.command.HQOrderItemCommand> orderItemByOrderItemId = orderItemsByOrderId
                                .values().stream()
                                .flatMap(List::stream)
                                .collect(Collectors.toMap(
                                ...
                                .flatMap(entry -> {
                                        ...
                                        return orderItemByProductId.entrySet().stream()
                                                        .map(entrySet -> {
                                                                ...
                                                                List<com.chaing.domain.orders.dto.command.HQOrderItemCommand> orderItems = orderItemIds
                                                                                .stream()
                                                                ...
                                                                Integer quantity = orderItems.stream()
                                                                                ...
                                                                                .sum();
                                                                ProductInfo productInfo = productInfoByProductId
                                                                                .get(productId);

                                                                if (productInfo == null) {
                                                                        throw new HQOrderException(
                                                                                        HQOrderErrorCode.PRODUCT_NOT_FOUND);
                                                                }
        ...
        public HQOrderDetailResponse getOrderDetail(String orderCode) {
                ...
                // Map<orderId, List<HQOrderItemCommand>>
                Map<Long, List<HQOrderItemCommand>> orderItemsByOrderId = hqOrderService
                                .getOrderItemsByOrderId(order.orderId());
                ...
                Map<Long, List<Long>> orderItemIdByProductId = orderItemsByOrderId.values().stream()
                                ...
                                .collect(Collectors.groupingBy(
                                                ...
                                                Collectors.mapping(HQOrderItemCommand::orderItemId,
                                                                Collectors.toList())));
        ...
        public HQOrderUpdateResponse updateOrder(Long userId, String orderCode, HQOrderUpdateRequest request) {
                ...
                // 발주 제품 데이터 수정
                List<HQOrderItemCommand> items = hqOrderService.updateOrderItems(userId, orderCode, request,
                                productInfoByProductCode);
        ...
        public HQOrderCreateResponse create(Long userId, HQOrderCreateRequest request) {
                ...
                // 발주 제품 생성 - 수정요망
                List<HQOrderItemCommand> items = hqOrderService.createOrderItems(order.orderId(),
                                productInfoByProductId, request.items());
        ...
        public List<HQRequestedOrderResponse> getRequestedOrders(boolean isPending) {
                ...
                Map<Long, List<FranchiseOrderItemCommand>> orderItemByOrderId = franchiseOrderService
                                .getAllRequestedOrderItem(orderIds);
                ...
                Map<Long, String> franchiseCodeByUserId = orders.values().stream()
                                .collect(Collectors.toMap(
                                                ...
                                                order -> {
                                                        try {
                                                                return franchiseService.getById(userManagementService
                                                                                .getFranchiseIdByUserId(order.userId()))
                                                                                .code();
                                                        } catch (Exception e) {
                                                                throw new HQOrderException(
                                                                                HQOrderErrorCode.INVALID_USER_INFO);
                                                        }
                                                },
                                                (a, b) -> a));
                ...
                Map<Long, String> receiverByOrderId = orders.values().stream()
                                .collect(Collectors.toMap(
                                                ...
                                                command -> userManagementService
                                                                .getUsernameByUserId(command.userId())));
                                ...
                                .flatMap(entry -> {
                                        ...
                                        return orderItemIdsByProductId.entrySet().stream()
                                                        .map(entrySet -> {
                                                                ...
                                                                String franchiseCode = franchiseCodeByUserId
                                                                                .get(orderUserId);

                                                                List<FranchiseOrderItemCommand> orderItems = orderItemIds
                                                                                .stream()
                                                                                .map(orderItemByOrderItemId::get)
                                                                                .toList();
                                                                ProductInfo productInfo = productInfoByProductId
