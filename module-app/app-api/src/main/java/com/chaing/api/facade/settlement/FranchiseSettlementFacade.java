...
public class FranchiseSettlementFacade {
    ...
    public FranchiseSettlementSummaryResponse getMonthlySummary(Long franchiseId, YearMonth month) {
        ...
        // 월별 일자 매출 추이 그래프
        public List<FranchiseDailyGraphResponse> getMonthlyDailyTrend (
                Long franchiseId, LocalDate start, LocalDate end){
            List<DailySettlementReceipt> receipts =
                    dailyService.getAllByFranchiseAndDateRange(franchiseId, start, end);
        ...
        public Page<FranchiseVoucherResponse> getVouchers (
            ...
            if (period == PeriodType.DAILY) {
                DailySettlementReceipt receipt =
                        dailyService.getByFranchiseAndDate(franchiseId, date);
                Page<DailyReceiptLine> lines =
                        dailyService.getReceiptLines(receipt.getDailyReceiptId(), type, pageable);
                return lines.map(this::toVoucherResponse);
            } else {
                // MONTHLY → SettlementVoucher 조회
                MonthlySettlement settlement =
                        monthlyService.getByFranchiseAndMonth(franchiseId, month);
                // TODO: SettlementVoucherRepository에서 조회
                // voucherRepository.findByMonthlySettlementId(settlement.getId(), type, pageable)
                return Page.empty();
            }
        }
