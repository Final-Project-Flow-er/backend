...
public class HQOrderService {
        ...
        public Map<Long, List<Long>> getAllOrderItemProductId(Long hqId, List<Long> orderIds) {
                List<HeadOfficeOrderItem> orderItems = orderItemRepository
                                .findAllByHeadOfficeOrder_HqIdAndHeadOfficeOrder_HeadOfficeOrderIdIn(hqId, orderIds);
        ...
        public List<HeadOfficeOrderItem> getOrderItemsByOrderId(Long orderId) {
                List<HeadOfficeOrderItem> items = orderItemRepository
        ...
        public List<Long> getOrderItemProductId(Long hqId, Long orderId) {
                List<HeadOfficeOrderItem> orderItems = orderItemRepository
                                .findAllByHeadOfficeOrder_HqIdAndHeadOfficeOrder_HeadOfficeOrderId(hqId, orderId);
        ...
        // 발주 제품 수정
        public List<HQOrderItemInfo> updateOrderItems(Long hqId, String orderCode,
                        List<HQOrderItemUpdateRequest> request, Map<Long, ProductInfo> productInfoByProductId) {
                // 발주 조회
                HeadOfficeOrder order = orderRepository.findByHqIdAndOrderCode(hqId, orderCode)
                                .orElseThrow(() -> new HQOrderException(HQOrderErrorCode.ORDER_NOT_FOUND));

                // Map<productId, HeadOfficeOrderItem> 발주 제품 조회
                Map<Long, HeadOfficeOrderItem> items = orderItemRepository
                                .findAllByHeadOfficeOrder_HqIdAndHeadOfficeOrder_OrderCodeAndDeletedAtIsNull(hqId,
                                                orderCode)
                                .stream()
                                .collect(Collectors.toMap(
                                                HeadOfficeOrderItem::getProductId,
                ...
                requestedItems.forEach((productId, quantity) -> {
                        if (!items.containsKey(productId)) {
                                ...
                                HeadOfficeOrderItem addedItem = HeadOfficeOrderItem.builder()
                                                ...
                                                .totalPrice(productInfo.costPrice()
        ...
        // 발주 생성
        public HQOrderInfo createOrder(Long hqId, HQOrderCreateRequest request, Integer totalQuantity,
        ...
        // 발주 제품 생성
        public List<HQOrderItemInfo> createOrderItems(Long orderId, Map<Long, ProductInfo> productInfoByProductId,
                ...
                List<HeadOfficeOrderItem> orderItems = items.stream()
                                .map(item -> HeadOfficeOrderItem.builder()
                                                ...
                                                .totalPrice(productInfoByProductId.get(item.productId()).costPrice()
        ...
        public Map<Long, List<Long>> getOrderItemIdsByOrderIdAndStatus(List<Long> orderIds) {
                List<HeadOfficeOrderItem> orderItems = orderItemRepository
                                .findAllByHeadOfficeOrder_HeadOfficeOrderIdIn(orderIds);
                ...
                return orderItems.stream()
                                .collect(Collectors.groupingBy(
                                                item -> item.getHeadOfficeOrder().getHeadOfficeOrderId(),
                                                Collectors.mapping(HeadOfficeOrderItem::getHeadOfficeOrderItemId,
                                                                Collectors.toList())));
        }
        ...
        public List<HQOrderForTransitResponse> getOrdersForTransit(List<Long> orderIds) {
                ...
                return itemsByOrder.entrySet().stream()
                                .map(entry -> {
                                        ...
                                        List<HQOrderForTransitResponse.OrderItemForTransit> itemResponses = items
                                                        .stream()
