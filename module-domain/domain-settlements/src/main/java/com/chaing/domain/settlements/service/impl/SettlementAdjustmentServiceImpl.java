...
public class SettlementAdjustmentServiceImpl implements SettlementAdjustmentService {
    ...
    @Override
    public Page<SettlementAdjustment> getAll(Long franchiseId, VoucherType voucherType, java.time.LocalDateTime startDate, java.time.LocalDateTime endDate, Pageable pageable) {
        return repository.findByConditions(franchiseId, voucherType, startDate, endDate, pageable);
    } // 조건별 조정 내역 DB에서 가져오기
